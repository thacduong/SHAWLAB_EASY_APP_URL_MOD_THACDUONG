  ...
  shiny::moduleServer(
    ...
    function(input, output, session) {
      ...
      shiny::observeEvent(input$button, {
        ...
        row_username <- which(dplyr::pull(data, {{user_col}}) == "user")
        ...
        if (length(row_username)) {
          ...
          if (sodium_hashed) {
            ...
            password_match <- identical(toupper(row_password), toupper(input$password))
          }
        } else {
        ...
        if (length(row_username) == 1 && password_match) {
          ...
          credentials$info <- dplyr::filter(data, {{user_col}} == "user")
          
          if (cookie_logins) {
            .sessionid <- randomString()
            shinyjs::js$setcookie(.sessionid)
            cookie_setter("user", .sessionid)
...
login <- function(input,
  ...
  shiny::observeEvent(input$button, {
    ...
    # check for match of input username to username column in data
    row_username <- which(dplyr::pull(data, !!users) == "user")
    ...
    if (length(row_username) == 1 && password_match) {
      ...
      credentials$info <- dplyr::filter(data, !!users == "user")
